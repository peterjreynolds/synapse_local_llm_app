...
internal fun Exception.toRemoteChatFailure(operation: String): RemoteChatException {
    val userMessage = when {
        ...
        this is FirebaseFunctionsException -> firebaseFunctionsFailureMessage(code, operation)
        else -> "Could not $operation. Try again."
    }
    return RemoteChatException(userMessage, this)
}

internal fun firebaseFunctionsFailureMessage(
    code: FirebaseFunctionsException.Code,
    operation: String,
): String = when (code) {
    FirebaseFunctionsException.Code.PERMISSION_DENIED -> "This account is not allowed to $operation."
    FirebaseFunctionsException.Code.UNAUTHENTICATED -> "Sign in again before trying to $operation."
    FirebaseFunctionsException.Code.UNAVAILABLE -> "Synapse Chat services are temporarily offline. Try again later."
    else -> "Could not $operation. Try again."
}
