...
import app.synapse.localllm.data.calling.AndroidDirectCallTerminalNotificationStore
...
class SynapseApplicationGraph private constructor(context: Context) {
    ...
    val directCallTerminalNotificationStore = AndroidDirectCallTerminalNotificationStore(applicationContext)
    ...
    val remoteChatSessionSynchronizer =
        RemoteChatSessionSynchronizer(
            directoryGateway = remoteDirectoryGateway,
            conversationGateway = remoteConversationGateway,
            cacheRepository = remoteChatCacheRepository,
            clock = clock,
        )
