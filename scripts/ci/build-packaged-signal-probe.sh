...
# Produces a separate instrumentation APK; never rewrites or re-signs the distribution APK.
: "${ANDROID_HOME:?ANDROID_HOME is required}"
: "${JAVA_HOME:?JAVA_HOME is required}"
export PATH="$JAVA_HOME/bin:$PATH"
repository_directory="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
probe_directory="$(mktemp -d -t synapse-packaged-signal-probe.XXXXXX)"
...
"$JAVA_HOME/bin/javac" --release 8 -Xlint:-options -classpath "$android_platform_jar" \
  ...
  "$repository_directory/privatechat/src/androidTest/java/app/synapse/privatechat/data/chat/PackagedSignalProbe.java"
...
"$build_tools_directory/aapt" package -f \
  -M "$repository_directory/scripts/ci/packaged-signal-probe/AndroidManifest.xml" \
